...
| 2026-07-02 | Antigravity | [星图退出按钮与2D纯色净化] 1. 星图页增加导航侦听，当滑动离开该页面时，Zustand 自动重置 is3DMode 为 false，彻底解决“退出按钮遗留在首页”的问题； 2. 2D 概览模式下 3D 星云背景自动透明淡出（opacity = 0），完美呈现 2D 知识树，告别双重背景重叠的杂乱感。 | npm run build 编译打包全部通过 |
