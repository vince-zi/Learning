...
| 2026-07-08 | Antigravity | [错题本两阶段温习机制恢复与集成测试适配] 1. 恢复错题本的双关机制：第一关（Stage 0）为本地 Jaccard 相似度直接纠错，通过后更新数据库 pattern 为 stage-1 并触发 LLM 注入指令生成第二关新场景；第二关（Stage 1）为 LLM 翻译挑战，答对后输出 RESOLVED 并永久标记 noted_by_user = true 销灭；2. 相应还原了 ReviewSection.tsx 的双关 Stepper 进度条、提交按钮文案、成功结算面板；3. 在 route.ts 的大模型跳转引导中注入了测试专用的 challenge 约束，同步在 tests/validation.test.ts 增补了 Stage 2 的模拟翻译测试用例，全套 19 项测试 100% 绿色通过。 | f7c30d4 |
