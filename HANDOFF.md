...
| 2026-07-08 | Antigravity | [修复结算卡死与顶部空白] 1. 并行化重构了 /api/discoveries 接口（并行读取DB、并行调用两段 AI 以及并行写入DB），卡片及画像评估生成整体时间从 10+s 缩短至 4s 左右；2. 对前端 PracticeSection.tsx 的 handleEndSession 逻辑引入 10 秒安全强制限时，防止后端异常时前端挂死；3. 调整对话主页面外层容器 pt-24 到 pt-12，消除顶部绝对定位的结束按钮上方的大片空白。 | 58639d0 |
