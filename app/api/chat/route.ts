...
Ask only one challenge. Keep it encouraging.${showChooseLangHint ? '\n\n⚠️ IMPORTANT: You MUST append exactly " [CHOOSE_LANG]" at the very end of your response to ask for language preference.' : ''}`
          ...
          const errorHintFirst = userHasEnglish
            ? (isEnglishPref
              ? `The user's message contains English. ⚠️ IMPORTANT: You MUST append exactly "[CORRECTED: <the fully corrected version of the user's sentence>]" at the very end of your response. The corrected sentence MUST be a complete standalone sentence with subject AND verb — never a fragment or phrase. It must be 100% grammatically correct native-level English.`
              : isBalancedPref
              ? `用户的发言中包含英文。⚠️ 重要：你必须在回复末尾确切地附加 "[CORRECTED: <用户句子的完整正确版本>]" 标签。纠正后的句子必须是完整的独立句（有主语+动词），不能是短语碎片，必须100%语法正确的地道英语。`
              : `用户在消息中使用了英文。⚠️ 重要：你必须在回复末尾确切地附加 "[CORRECTED: <用户句子的完整正确版本>]" 标签。纠正后的句子必须是完整的独立句（有主语+动词），不能是短语碎片，必须100%语法正确的地道英语。`)
            : ''
...
每次只问一个问题，引导他们发现背后的规律。`
      ...
      const correctedInstruction = /[a-zA-Z]{3,}/.test(userMessage)
        ? `\n⚠️ IMPORTANT: You MUST append exactly "[CORRECTED: <the fully corrected version of the user's sentence>]" at the very end of your response. The corrected sentence MUST be a complete standalone sentence with subject AND verb — never a fragment or phrase. It must be 100% grammatically correct, natural, native-level English.`
        : ''
    ...
    if (isReviewMode && targetErrorToReview) {
      ...
      if (userMessage === 'I am ready to review.') {
        if (targetErrorStage === 1) {
          const rawScenario = targetErrorToReview.review_scenario || '【新场景挑战】请用英文翻译以下句子：”我真的很喜欢学英语。”'
          // review_scenario 格式： “中文场景描述 | English answer”，只提取中文部分展示给用户
          const scenarioPartsForDisplay = rawScenario.split('|')
          const cnScenario = scenarioPartsForDisplay[0]?.trim() || rawScenario
          const cleanScenario = cnScenario.replace(/【新场景挑战】请用英文翻译以下句子：|”|”/g, '')
...
请结合上面的高亮词汇与语法结构公式，在对话框中用正确的英文重新改写这句错句吧！`
        ...
        if (Jaccard >= 0.75) {
          ...
          // 更新数据库，将错句推进到 stage-1，存储挑战题目与对应的正确翻译
          // 注意：review_scenario 同时存储中文场景 | 英文答案（用 | 分隔），corrected_text 保留原始正确句不被覆写
          const newPattern = `${targetErrorToReview.error_pattern || ''}:stage-1`
          const reviewScenarioWithAnswer = `${challengeCn} | ${challengeEn}`
          await safeUpdateErrorRecord(supabase, targetErrorToReview.id, {
            error_pattern: newPattern,
            review_scenario: reviewScenarioWithAnswer,
          })
...
${challengeCn.startsWith('【新场景挑战】') ? challengeCn.replace('【新场景挑战】', '') : `请将以下句子翻译为英文：\n“${challengeCn}”`}`
        ...
        // Stage 1 评估：从 review_scenario 中提取挑战的正确答案（格式："中文场景 | English answer"）
        // 不再依赖 corrected_text（corrected_text 保留原始纠错结果，不被挑战答案覆写）
        const scenarioParts = (targetErrorToReview.review_scenario || '').split('|')
        const challengeAnswer = scenarioParts[1]?.trim() || corrected
        let correctOptions = [challengeAnswer]
        const errKey = targetErrorToReview.error_type || 'expression-chinglish'
        const challenges = LOCAL_TRANSLATION_CHALLENGES[errKey] || LOCAL_TRANSLATION_CHALLENGES['expression-chinglish']
        const matchingChallenge = challenges.find(c => c.en === challengeAnswer || (targetErrorToReview.review_scenario && targetErrorToReview.review_scenario.includes(c.cn)))
        ...
        let maxJaccard = 0
        let bestOption = challengeAnswer
...
[RESOLVED: ${orig}]`
          ...
          // Stage 1 失败时：用 challengeAnswer（挑战答案）而不是 corrected（原始纠错句）做对比
          // targetText 只传中文场景部分，不暴露英文答案
          const cnScenarioForError = (targetErrorToReview.review_scenario || '').split('|')[0]?.trim() || '请用正确表达回答'
          localAiResponse = generateStepByStepCritique(userMessage, challengeAnswer, targetErrorToReview.error_type || 'expression-chinglish', cnScenarioForError, typeName, true)
          isLocalReviewResponse = true
        }
      }
    }
